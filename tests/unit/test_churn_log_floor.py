"""A laid-down churn log is re-dated only down to the floor it was cut at.

git reads "N months ago" with month arithmetic, so the window floor can move
back overnight: 6 months before Aug 31 is Mar 3, and before Sep 1 it is Mar 1.
A log cut at the higher floor lacks the commits between the two, and re-dating
it cannot bring them back. So the log's key records the floor its walk was cut
at, the walk is cut at exactly that floor (`--max-age`), and a refresh below
it walks the window instead.
...
class FakeGit:
    def __init__(self):
        ...
        self.floor: int | None = 999999999
        self.walked_at: list[int | None] = []  # the floor each window walk was cut at
        self.range_calls: list[tuple[str, str]] = []

    def window(self, root, months, head, cutoff):
        self.walked_at.append(cutoff)
        return iter(since(self.log, self.floor if cutoff is None else cutoff))
...
def git(monkeypatch) -> FakeGit:
    ...
    monkeypatch.setattr(churn_log, "_window_cutoff", lambda root, months: fake.floor)
...
def test_the_walk_is_cut_at_the_floor_the_stamp_records(tmp_path, git):
    git.floor = 1000000100

    window = churn_log.stored_window(tmp_path, 12, None)

    assert list(window.lines) == MID, "alice's commit sits below the floor"
    assert window.cutoff == 1000000100
    assert git.walked_at == [1000000100], "walked at the floor read, not at a second reading"
    assert stamp(tmp_path)["cutoff"] == 1000000100


def test_a_floor_moved_back_walks_the_window_again(tmp_path, git, monkeypatch):
    ...
    assert stored(tmp_path) == LOG, "the cold walk at the lower floor has alice's commit"
    assert git.walked_at == [1000000100, 1000000000]
    assert stamp(tmp_path)["cutoff"] == 1000000000


def test_a_floor_moved_forward_re_dates_the_log_without_a_walk(tmp_path, git, monkeypatch):
...
def test_a_log_that_recorded_no_floor_is_walked_not_refreshed(tmp_path, git):
    """A log laid down before the floor was recorded: nothing says how far
...
def test_a_served_log_that_recorded_no_floor_names_none(tmp_path, git):
    stored(tmp_path)
    doc = stamp(tmp_path)
    doc["cutoff"] = "not a floor"
    churn_log._key_path(tmp_path / ".crapkit" / churn_log.LOG_NAME).write_text(
        json.dumps(doc), encoding="utf-8")
...
def test_a_floor_git_will_not_name_walks_by_the_clock(tmp_path, git):
    stored(tmp_path)
    git.head = HEAD_B
    git.ranges[(HEAD_A, HEAD_B)] = list(NEW)
    git.log = NEW + LOG
    ...
    assert git.walked_at[-1] is None, "no floor to cut at: git reads --since itself"
    assert git.range_calls == []
