...
changing behavior. The sketches are generic on purpose: match the shape, not the names.

## Subtract first, always

Deleting beats extracting, so delete before you cut. A branch nothing reaches, a flag one
caller passes, a validator the boundary already ran: each deletion drops ccn and adds no
name to the file. An extraction moves complexity; a deletion removes it. Extract only what
survives the cut.
...
Every `elif` is another branch. A mapping is one lookup however long it gets, and a new case
stops touching the function at all.
...
A loop that decides as well as iterates holds two jobs. Leave the iteration where it is and
move the decision into a function that takes one element and returns one result.
...
met by moving the mess rather than removing it. That is a redesign. Name it as one, say what
the new seam would be, and stop instead of shipping a split that regrows.
