...
| `mutation_workers` | int >= 1 | `1` | Mutants run at once. `1` applies them to the live working tree one at a time. Above 1, crapkit runs each worker in its own detached git worktree and deals mutants round-robin. Those worktrees are **kept**, at `.crapkit/mutate-pool/w0..wN`: building four of them costs 35.5 s on a 31,459-file repo and re-preparing the kept four costs 0.44 s, and every run re-prepares them (`git checkout --force <HEAD sha>`, then `git clean -xdff`) before a mutant is applied. What stays on disk is that many checkouts of your repo; `crapkit mutate --drop-pool` removes them. A second `mutate` running in the same repo finds the pool locked and gets its own throwaway worktrees, removed on the way out as before. |
